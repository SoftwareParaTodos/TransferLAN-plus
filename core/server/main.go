...
func startUDPDiscovery() {
	...
	if err != nil {
		log.Println("Aviso: UDP discovery no disponible (puerto ocupado). HTTP sigue funcionando:", err)
		return
	}
	defer conn.Close()
	buf := make([]byte, 2048)
...
.card{max-width:1040px;margin:auto;background:rgba(15,23,42,.92);border:1px solid rgba(148,163,184,.25);border-radius:28px;padding:28px;box-shadow:0 24px 80px #000b}
...
.ok{color:#22c55e;font-weight:900}.warn{color:#fbbf24;font-weight:900}.small{color:#94a3b8;font-size:14px}.pair{font-size:13px;min-height:110px}
...
<div class="box"><h2>Emparejar Android</h2><p>Copiá este código y pegalo en Android en <b>Pegar código de PC</b>.</p><textarea class="pair" id="pair">%s</textarea><button onclick="copyPair()">Copiar código</button></div>
...
<div class="box"><h2>Subir desde navegador</h2><input type="file" id="file"><br><button onclick="upload()">Subir archivo</button><pre id="out">Esperando...</pre></div>
<div class="box"><h2>Diagnóstico</h2><button onclick="check('/health')">/health</button><button onclick="check('/device/info')">/device/info</button><button onclick="check('/history')">/history</button><pre id="diag">Sin verificar</pre></div>
</div></div>
<script>
async function upload(){const f=document.getElementById('file').files[0];if(!f){alert('Elegí un archivo');return}const fd=new FormData();fd.append('file',f);out.textContent='Subiendo...';const r=await fetch('/transfer/upload',{method:'POST',body:fd});out.textContent=await r.text();loadHistory();}
...
</script></body></html>`, version, ip, pair)
}
