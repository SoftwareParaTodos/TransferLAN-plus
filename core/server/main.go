...
.card{max-width:980px;margin:auto;background:rgba(15,23,42,.92);border:1px solid rgba(148,163,184,.25);border-radius:28px;padding:28px;box-shadow:0 24px 80px #000b}
...
.pair{font-size:13px;min-height:120px}
</style>
</head>
<body>
<div class="card">
...
<p>Desde esta PC:</p><code>http://localhost:5050</code>
<p>Desde Android:</p><code>http://%s:5050</code>
</div>

<div class="box">
<h2>Emparejar Android</h2>
<p>Copiá este código/link y pegalo en Android en <b>Pegar código de PC</b>.</p>
<textarea class="pair" id="pair">%s</textarea>
<button onclick="copyPair()">Copiar código</button>
<p class="small">Esto reemplaza temporalmente el scanner QR para evitar cierres de la app.</p>
</div>

<div class="box">
<h2>Datos de emparejamiento</h2>
<button onclick="check('/pairing/info')">/pairing/info</button>
<button onclick="check('/pairing/code')">/pairing/code</button>
...
<br><button onclick="upload()">Subir archivo</button>
...
async function check(path){
 ...
 if(ct.includes('application/json')) diag.textContent=JSON.stringify(await r.json(),null,2);
 else diag.textContent=await r.text();
}
function copyPair(){
